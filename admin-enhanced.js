...
function generateCodeFiles() {
  ...
  if (data.certificates && data.certificates.length > 0) {
    const certsCode = generateCertificatesCode(data.certificates);
    showCodePreview('certificates', certsCode);
    downloadCodeFile('certificates-data.js', certsCode);
  }
  
  // Generate projects-data.js (if projects were modified)
  if (data.projects && data.projects.length > 0) {
    const projectsCode = generateProjectsCode(data.projects);
    showCodePreview('projects', projectsCode);
    downloadCodeFile('projects-data-updated.js', projectsCode);
  }
}
